    ...
    if (sz == 1)                  // must have byte regs
    {
        ...
        if (!retregs)
           ...
           (
             ...
             (e1.Eoper == OPvar && e1.Vsym.Sfl == FL.fardata)
           )
          )
...
Lp:
    if (postinc)
    {
        ...
        {   // Use LEA to avoid touching the flags
            ...
            if (cs.Irex & REX_B)
                rm |= 8;
            cdb.genc1(LEA,buildModregrm(2,ireg,rm),FL.const_,postinc);
...
void cdshass(ref CGstate cg, ref CodeBuilder cdb,elem* e,ref regm_t pretregs)
{
    ...
    if (e2.Eoper == OPconst)
    {
        ...
        shiftcnt = e2.Vint;         // byte ordering of host
        if (config.target_cpu >= TARGET_80286 &&
            sz <= REGSIZE &&
            shiftcnt != 1)
...
void cdcmp(ref CGstate cg, ref CodeBuilder cdb,elem* e,ref regm_t pretregs)
{
    ...
    switch (e2.Eoper)
    {
        ...
        case OPrelconst:
            ...
            {
                case FL.func:
                    fl = FL.extern_;          // so it won't be self-relative
                    break;
        ...
        case OPconst:
          ...
          L4:
            ...
            if (sz == 1)
            {
                ...
                if (reg & 8)
                    cs.Irex |= REX_B;
                if (e1.Eoper == OPvar && e1.Voffset == 1 && e1.Vsym.Sfl == FL.reg)
                {   assert(reg < 4);
...
void longcmp(ref CodeBuilder cdb, elem* e, bool jcond, FL fltarg, code* targ)
{
    ...
    switch (e2.Eoper)
    {
        ...
        case OPconst:
            cs.IEV2.Vint = cast(int)MSREG(e2.Vllong);            // MSW first
            cs.IFL2 = FL.const_;
            cs.Iop = 0x81;
...
//    if (e2.Eoper == OPconst && e2.Vuns < 0x100)  // should do this instead?
    if (e2.Eoper == OPconst)
    {
        ...
        if (sz <= SHORTSIZE)
        {
            cs.IEV2.Vint = e2.Vint & 15;
        }
        else if (sz == 4)
        {
            cs.IEV2.Vint = e2.Vint & 31;
        }
        else
        {
            cs.IEV2.Vint = e2.Vint & 63;
            if (I64)
                cs.Irex |= REX_W;
        }
        cdb.gen(&cs);
...
//    if (e2.Eoper == OPconst && e2.Vuns < 0x100)  // should do this instead?
    if (e2.Eoper == OPconst)
    {
        ...
        {
            cs.IEV1.Voffset += (e2.Vuns & ~15) >> 3;
            cs.IEV2.Vint = e2.Vint & 15;
        }
        else if (_tysize[ty1] == 4)
        {
            cs.IEV1.Voffset += (e2.Vuns & ~31) >> 3;
            cs.IEV2.Vint = e2.Vint & 31;
        }
        else
        {
            cs.IEV1.Voffset += (e2.Vuns & ~63) >> 3;
            cs.IEV2.Vint = e2.Vint & 63;
            if (I64)
                cs.Irex |= REX_W;
        }
